...
impl Assembler {
    ...
    pub fn ldrb_pre_index(&mut self, out: Reg, base: Reg, offset: Imm) {
        assert!(offset.as_isize() < 256);
        assert!(offset.as_isize() >= -256);
        self.instrs.push(Instr::LdrbImmPreIndex(out, base, offset));
    }
    ...
    pub fn mov(&mut self, out: Reg, inp: Reg) {
        let has_sp = out == Reg::SP || inp == Reg::SP;

        if has_sp {
            self.add_imm(out, inp, 0.into());
            return;
        }
    ...
    pub fn emit_binary(
        ...
        match instr {
            ...
            Instr::LdrhImm(out, base, offset) => {
                assert!(out.is_32bit());
                assert!(base.is_64bit());
                assert!(offset.as_usize() <= 8190);
                assert!(offset.as_usize() % 2 == 0);

                let mut instrbits = LDRH_IMM;
                instrbits.set_bit_range(4, 0, out.as_bits());
                instrbits.set_bit_range(9, 5, base.as_bits());
                instrbits.set_bit_range(21, 10, offset.as_u32() / 2);
            ...
            Instr::LdrbImmPreIndex(out, base, offset) => {
                assert!(offset.as_isize() < 256);
                assert!(offset.as_isize() >= -256);
                let out_bits = out.as_bits();
                let base_bits = base.as_bits();
                let mut instrbits = LDRB_IMM_PRE_INDEX;
                instrbits.set_bit_range(4, 0, out_bits);
                instrbits.set_bit_range(9, 5, base_bits);
                instrbits.set_bit_range(20, 12, offset.as_isize() as u32);
...
mod test {
    ...
    #[test]
    fn test_assembler_ldrh() {
        use Reg::*;

        // Test basic unsigned offset ldrh: loads halfword (2 bytes) from [base + offset]
        // Does NOT update base register (unlike pre/post-index)
        {
            let mut assembler = Assembler::new();
            // ldrh w0, [x0, #4]  - load halfword from X0+4
            assembler.ldrh_imm(W0, X0, Imm::U32(4));
            assembler.ret();
            ...
            // At offset 4: 0xEE, 0xFF -> 0xFFEE (little-endian)
            let input = [0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF];
            let result = func(input.as_ptr());
            assert_eq!(result, 0xFFEE);
        }

        // Test with zero offset
        {
            let mut assembler = Assembler::new();
            // ldrh w0, [x0, #0]  - load halfword from X0
            assembler.ldrh_imm(W0, X0, Imm::U32(0));
            assembler.ret();
        ...
        // Test multiple loads at different offsets
        {
            let mut assembler = Assembler::new();
            // Load three halfwords and sum them
            assembler.ldrh_imm(W1, X0, Imm::U32(0)); // offset 0
            assembler.ldrh_imm(W2, X0, Imm::U32(2)); // offset 2
            assembler.ldrh_imm(W3, X0, Imm::U32(4)); // offset 4
                                                     // Sum: W0 = W1 + W2 + W3
            ...
            // Input: [0x01, 0x00, 0x02, 0x00, 0x03, 0x00]
            // offset 0: 0x0001
            // offset 2: 0x0002
            // offset 4: 0x0003
            // sum = 1 + 2 + 3 = 6
            let input = [0x01, 0x00, 0x02, 0x00, 0x03, 0x00];
            let result = func(input.as_ptr());
            assert_eq!(result, 6);
        }

        // Test that base register is NOT modified (unlike pre/post-index)
        {
            let mut assembler = Assembler::new();
            // Save original pointer
            assembler.mov(X1, X0);
            // Load with offset
            assembler.ldrh_imm(W2, X0, Imm::U32(8));
            // Calculate difference (should be 0 since base isn't modified)
        ...
        // Test larger offset (max is 8190, must be even)
        {
            let mut assembler = Assembler::new();
            // ldrh w0, [x0, #100]
            assembler.ldrh_imm(W0, X0, Imm::U32(100));
            assembler.ret();
            ...
            // Set halfword at offset 100 to 0xABCD
            input[100] = 0xCD;
            input[101] = 0xAB;
            let result = func(input.as_ptr());
        ...
        // Test zero-extension: verify the loaded halfword is zero-extended to 32 bits
        {
            let mut assembler = Assembler::new();
            // Load halfword from X0 (should zero-extend to 32 bits)
            assembler.ldrh_imm(W0, X0, Imm::U32(0));
            // Shift left by 16 - if zero-extended properly, upper bits should be 0
            assembler.add_shift(W0, W0, W0, ShiftKind::Lsl, Imm::U32(16));
            assembler.ret();
    ...
    #[test]
    fn test_assembler_ldrb_imm_pre_index() {
        use Reg::*;

        // Test basic pre-index: loads byte from [base + offset], updates base to base + offset
        {
            let mut assembler = Assembler::new();
            // ldrb w1, [x0, #4]!  - load byte from X0+4, update X0 to X0+4
            assembler.ldrb_pre_index(W1, X0, Imm::I16(4));
            ...
            // Input array: [0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22]
        ...
        // Test with negative offset
        {
            ...
            // Start at offset 10
            assembler.add_imm(X0, X0, Imm::U32(10));
            // ldrb w1, [x0, #-5]!  - load byte from X0-5, update X0 to X0-5
            assembler.ldrb_pre_index(W1, X0, Imm::I16(-5));
            // Return loaded byte
            assembler.mov(W0, W1);
            assembler.ret();
            ...
            // Start at input[10], go back 5 to input[5] = 0x66
            let result = func(input.as_ptr());
            assert_eq!(result, 0x66);
        }

        // Test that base register is properly updated
        {
            let mut assembler = Assembler::new();
            // X1 = X0 (save original pointer)
            assembler.mov(X1, X0);
            // ldrb w2, [x0, #8]!  - load byte, update X0
            assembler.ldrb_pre_index(W2, X0, Imm::I16(8));
            // X0 = X0 - X1 (calculate the change in X0)
            assembler.subs_ext_reg(X0, X0, X1);
            // Return the offset (should be 8)
            assembler.ret();
            ...
            let input = [0u8; 20];
            let result = func(input.as_ptr());
            assert_eq!(result, 8); // Base register was incremented by 8
        }

        // Test sequential pre-indexed loads to verify base updates correctly
        {
            let mut assembler = Assembler::new();
            // Load bytes with pre-indexing: [x0, #1]!, [x0, #1]!, [x0, #1]!
            assembler.ldrb_pre_index(W1, X0, Imm::I16(1));
            assembler.ldrb_pre_index(W2, X0, Imm::I16(1));
            assembler.ldrb_pre_index(W3, X0, Imm::I16(1));
            // Combine: W0 = W1 + W2 + W3
            assembler.add(W0, W1, W2);
            assembler.add(W0, W0, W3);
            assembler.ret();
            ...
            let input = [10, 20, 30, 40, 50];
            // Loads input[1]=20, input[2]=30, input[3]=40
            // Sum = 20 + 30 + 40 = 90
            let result = func(input.as_ptr());
            assert_eq!(result, 90);
        }
